...
func tripleToBinary(triplet string) string {
	if triplet == "rwx" {
		return "111"
	}
	if triplet == "-wx" {
...
func convertToBonary(permissions string) string {
	p1 := permissions[0:3]
	p2 := permissions[3:6]
	p3 := permissions[6:9]

	p1 = tripleToBinary(p1)
	p2 = tripleToBinary(p2)
	p3 = tripleToBinary(p3)

	p1Int, _ := strconv.ParseInt(p1, 2, 64)
	p2Int, _ := strconv.ParseInt(p2, 2, 64)
	p3Int, _ := strconv.ParseInt(p2, 2, 64)
...
func main() {
	...
	bin := convertToBonary(permissions)
