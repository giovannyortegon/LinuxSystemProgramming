...
int main() {
    ...
    * ju = 13;
    ...
    if (reinterpret_cast<unsigned long>(iu) % 8 == 0)
        ...
        std::cout <<"memory pointed by the <iu> variable NOT aligned to 8 byte" <<std::endl;

    if (reinterpret_cast<unsigned long>(ju) % 8 == 0)
        std::cout <<"memory pointed by the <ju> variable aligned to 8 byte" <<std::endl;
    else
        std::cout <<"memory pointed by the <ju> variable NOT aligned to 8 byte" <<std::endl;
